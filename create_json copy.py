...
def create_json(url,loc):
    ...
    label_elements=soup.find_all('meta',attrs={'name':'keywords'})
    ...
    if not len(label_elements)==1:
        print('!!!!!!!!!Error:label_lenth != 1!!!!!!')
        warning_name=loc+'/'+data['title']
        warning_res['labels']=warning_name
        return warning_res
    else:
        try:
            labels=[i.strip() for i in label_elements['content'].split(',')]
            data['labels']=labels
        except:
